...
MODEL_BUILD_UTIL_MAP = {
    ...
    'create_predict_input_fn':
        inputs.checkpoint_create_predict_input_fn,
}
...
def create_model_fn(detection_model_fn, configs, hparams, use_tpu=False):
  ...
  def model_fn(features, labels, mode, params=None):
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
      detections[fields.InputDataFields.source_id] = features[fields.InputDataFields.source_id]
...
def create_estimator_and_inputs(run_config,
  ...
  predict_input_fn = create_predict_input_fn(
      model_config=model_config, predict_config=eval_config, predict_input_config=eval_input_configs[0])
...
def predict_examples(estimator, predict_input_fn, checkpoint_dir):
    predictions = estimator.predict(
            predict_input_fn,
            checkpoint_path=tf.train.latest_checkpoint(checkpoint_dir))

    local_tz = tzlocal()
    timestamp = datetime.now(local_tz).strftime('%m_%d_%Y_%H_%M_%S')

    filename = 'test_detections_{}.json'.format(timestamp)
    json_output_path = os.path.join(checkpoint_dir, filename)

    print('detections will be written to \"{}\"'.format(json_output_path))

    dict_predictions = {}

    idata = 0
    for prediction in predictions:
        source_id = prediction[fields.DetectionResultFields.source_id]
        print('predict \"{}\" was done. i={}.'.format(source_id, idata))
        idata += 1

        for key, value in prediction.items():
            if not key in dict_predictions.keys():
                dict_predictions[key] = []

            if key == fields.DetectionResultFields.num_detections:
                value = int(value)
            elif key == fields.DetectionResultFields.detection_boxes: 
                value = value.tolist()
            elif key == fields.DetectionResultFields.detection_scores: 
                value = value.tolist()
            elif key == fields.DetectionResultFields.detection_classes: 
                value = value.astype(np.uint8).tolist()

            dict_predictions[key].append(value)

    with tf.gfile.GFile(json_output_path, 'w') as fid:
        json_utils.Dump(
            obj=dict_predictions, fid=fid, float_digits=4, indent=2)
