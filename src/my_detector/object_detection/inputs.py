...
def _get_features_dict_predict(input_dict):
    features = _get_features_dict(input_dict)
    del features[HASH_KEY]
...
def checkpoint_create_predict_input_fn(model_config, predict_config, predict_input_config):
  """Creates a predict `input` function for `Estimator`.
  ...
  def _predict_input_fn(params=None):
    ...
    """
    del params
    if not isinstance(predict_config, eval_pb2.EvalConfig):
      raise TypeError('For predict mode, the `predict_config` must be a '
                      'train_pb2.EvalConfig.')
    if not isinstance(predict_input_config, input_reader_pb2.InputReader):
      raise TypeError('The `predict_input_config` must be a '
                      'input_reader_pb2.InputReader.')
    if not isinstance(model_config, model_pb2.DetectionModel):
      raise TypeError('The `model_config` must be a '
                      'model_pb2.DetectionModel.')
    ...
    def transform_and_pad_input_data_fn(tensor_dict):
      ...
      transform_data_fn = functools.partial(
          ...
          num_classes=num_classes,
          data_augmentation_fn=None,
          retain_original_image=predict_config.retain_original_images)
      tensor_dict = pad_input_data_to_static_shapes(
          tensor_dict=transform_data_fn(tensor_dict),
          max_num_boxes=predict_input_config.max_number_of_boxes,
          num_classes=config_util.get_number_of_classes(model_config),
          spatial_image_shape=config_util.get_spatial_image_size(
              image_resizer_config))

      return _get_features_dict_predict(tensor_dict)

    dataset = INPUT_BUILDER_UTIL_MAP['dataset_build'](
        predict_input_config, batch_size=1, transform_input_data_fn=transform_and_pad_input_data_fn)
    return dataset
